...
void ImuProcess::UndistortRawPcl(const typename Cloud::Ptr &meas_lidar, const esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, typename Cloud::Ptr & cl_out )
{
  if ( ! meas_lidar || IMUpose.empty() || imu_need_init_ ) return;
...
template void ImuProcess::UndistortRawPcl<PointCloudOuster>(const typename PointCloudOuster::Ptr &meas_lidar, const esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, typename PointCloudOuster::Ptr &pcl_out);
template void ImuProcess::UndistortRawPcl<PointCloudHesai>(const typename PointCloudHesai::Ptr &meas_lidar, const esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, typename PointCloudHesai::Ptr &pcl_out);
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  ROS_INFO_STREAM("[ IMU Process ]: Process lidar from "<<pcl_beg_time<<" to "<<(pcl_end_time-pcl_beg_time)<<", " \
            <<meas.imu.size()<<" imu msgs from "<<(imu_beg_time-pcl_beg_time)<<" to "<<(imu_end_time-pcl_beg_time) << " d: " << (imu_beg_time < pcl_beg_time) << " v: " << (imu_end_time<pcl_end_time) );
  ...
  //ROS_INFO_STREAM("dt: " << dt << " e: " << note);

  /*** undistort each lidar point (backward propagation) ***/
  if ( dont_compensate_ ) return;
  if (pcl_out.points.begin() == pcl_out.points.end()) return;
  auto it_pcl = pcl_out.points.end() - 1;
  if constexpr ( print_info ) ROS_INFO_STREAM("last: " << it_pcl->curvature);
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--)
  {
    auto head = it_kp - 1;
    auto tail = it_kp;
    ...
    for(; it_pcl->curvature / double(1000) > head->offset_time; it_pcl --)
    {
      ...
      //std::cout << "IMUProcess: pt_it=" << it_pcl->intensity << ", pt_ref=" << it_pcl->reflectance << std::endl;
